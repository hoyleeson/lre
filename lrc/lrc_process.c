...

    {
        ...
        if (process->procpath) {
            ...
            if (l > 32) {
                ...
                p = process->procpath;
            len += xsnprintf(buf + len, DETAILS_UNIT_MAX - len, " '%s%s'",
                             omit ? "..." : "", p);
        }
...
static int expr_running_handler(lrc_obj_t *handle, int opt,
                                struct lre_value *lreval)
{
    ...
    if (vaild_lre_results(ret)) {
        ...
        char buf[DETAILS_UNIT_MAX] = {0};
        int len;
        len = xsnprintf(buf, DETAILS_UNIT_MAX, "%srunning.",
                        process->count ? "" : "not ");
...
static int expr_user_handler(lrc_obj_t *handle, int opt,
                             struct lre_value *lreval)
{
    int i;
    int ret;
