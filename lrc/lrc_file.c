...
static int expr_exist_handler(lrc_obj_t *handle, int opt,
...
static int expr_owner_handler(lrc_obj_t *handle, int opt,
                              struct lre_value *lreval)
{
    const char *str;
    struct lrc_file *file;
...
static int expr_permission_handler(lrc_obj_t *handle, int opt,
                                   struct lre_value *lreval)
{
    int ret;
    unsigned int val;
    ...
    if (vaild_lre_results(ret)) {
        int len;
        char buf[DETAILS_UNIT_MAX] = {0};
        len = xsnprintf(buf, DETAILS_UNIT_MAX, "'permission' %smatched.",
                        ret ? "" : "not ");
        if (!ret) {
